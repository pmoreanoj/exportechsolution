...
(function ($, undefined) {
	...
	function Gallery() {
		...
		this.messages = {
			...
			compression_title: "Compress image",
			...
			upload: "Upload",
			watermark: "Watermark",
			watermark_title: "Watermark image",
	...
	Gallery.prototype = {
		_attachGallery: function (target, settings) {
				...
				.dialog({
					...
					open: function () {
						...
						$.get(inst.settings.updateUrl, {
							...
							$p = $("<p>");
							$("<label>").addClass("title").text(self.messages.watermark_text).appendTo($p);
							$("<input>", {
								"type": "text",
								"name": "watermark",
			...
			$("<div>", {
				...
				"title": self.messages.watermark_title
			})
				.hide()
				.insertAfter($target)
				.dialog({
			...
			$("<div>", {
				...
				"title": self.messages.compression_title
			})
				.hide()
				.insertAfter($target)
				.dialog({
