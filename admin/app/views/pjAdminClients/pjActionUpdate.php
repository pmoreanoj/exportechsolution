...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminClients&amp;action=pjActionUpdate" method="post" class="pj-form form" id="frmUpdateClient">
		...
		<fieldset class="fieldset white">
			...
			<p>
				<span class="pj-form-field-custom pj-form-field-custom-before">
					<label class="title">Tax Exempt</label>
					<?php
						$pjClientModel = new pjClientModel();
						$query = $pjClientModel
									->select('tax_exempt')
									->where('email', $tpl['arr']['email'])
									->findAll()
									->getData();
						$tax = $query[0]['tax_exempt'];
						if($tax == 'F' || $tax == ''){
							echo "<input type=\"radio\" name=\"tax_exempt\" value=\"T\" style=\"margin-top: 11px;\">Yes
								  &nbsp&nbsp&nbsp&nbsp
								  <input type=\"radio\" name=\"tax_exempt\" value=\"F\" style=\"margin-top: 11px;\" checked>No";
						}else{
							echo "<input type=\"radio\" name=\"tax_exempt\" value=\"T\" style=\"margin-top: 11px;\" checked>Yes
							      &nbsp&nbsp&nbsp&nbsp
							      <input type=\"radio\" name=\"tax_exempt\" value=\"F\" style=\"margin-top: 11px;\">No";	
						}
					?>
				</span>
			</p>
