...
class pjAdminUsers extends pjAdmin
{
	...
	public function pjActionCreate()
	{
		...
		if (in_array($this->getRoleId(), array(1,2)))  //change using this role id
		{
			...
			{
				$data = array();
				$data['role_id'] = $_POST['role'];//this variable is dynamic
				$data['is_active'] = 'T';
	...
	public function pjActionDeleteUser() //delete a single user
	{
		$this->setAjax(true);
		...
		{
			$response = array();
			if (!in_array($_GET['id'], array(1, $this->getUserId())))
			{
	...
	public function pjActionDeleteUserBulk() //delete all users (except user with id=1 in this case Felipe) 
	{
		$this->setAjax(true);
	...
	public function pjActionExportUser() // function to export users in CSV format
	{
		$this->checkLogin();
	...
	public function pjActionGetUser() //function to show active users registered in the admin panel.
	{
		$this->setAjax(true);
	...
	public function pjActionUpdate()
	{
		...
		if (in_array($this->getRoleId(), array(1,2)))  //change using this role id
		{
			if (isset($_POST['user_update']))
			{
				...
				$this->set('array', $arr);		//Attempt to get user_role
				if (count($arr) === 0)
				{
